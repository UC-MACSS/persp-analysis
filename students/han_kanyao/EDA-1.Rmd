...
```{r, echo = FALSE}
gss %>%
  drop_na(age_5, educ_4) %>%
  ggplot(aes(educ_4, ..prop.., group = 1)) +
  geom_bar(show.legend = FALSE) +
  labs(title = "Graph 1: Education Distribution",
       x = "Education",
       y = "Proportion") +
  facet_wrap(~ age_5) +
  coord_flip()
...
ggplot(gss, aes(as.integer(income06), color = educ_4)) +
  geom_density() +
  labs(title = "Graph 2: Distribution of Income",
       x = "Income Level",
       y = "Density")
```
...
ggplot(gss1, aes(as.integer(educ), as.integer(income06))) +
  geom_jitter(alpha = 0.4, color = "sky blue") +
  geom_smooth(se = FALSE, color = "orange") +
  geom_smooth(se = FALSE,method = "lm", color = "purple") +
  labs(title = "Graph 3: Relationship between Income Level and Education Years",
       x = "Education Years",
       y = "Income Level") +
  geom_text(data = gss1 %>%
  summarize(rsq = format(cor(as.integer(educ), as.integer(income06)))),
  aes(x = 3, y = 25, label = rsq))

gss1 %>%
  drop_na(age_5, income06) %>%
  ggplot(aes(as.numeric(educ), as.integer(income06))) +
  geom_jitter(alpha = 0.4, color = "sky blue") +
  geom_smooth(se = FALSE, color = "purple", method = "lm") +
  facet_grid(age_5 ~ race) +
  labs(title = "Graph 4: Relationship between Income Level and Education Years",
       x = "Education Years",
       y = "Income Level") +
  geom_text(data = gss1 %>%
              group_by(race, age_5) %>%
              summarize(rsq = format(cor(as.integer(educ), as.integer(income06)))),
  aes(x = 8, y = 25, label = rsq), size = 3)
```
...
gss3 %>%
  ...
  geom_jitter(alpha = 0.2) +
  geom_smooth() +
  labs(title = "Graph 5: Relationship between Family Member Number and Education Years",
       x = "Education Years",
       y = "Number")
...
gss %>%
  ...
  ggplot(aes(type, number, fill = age_5)) +
  geom_col(position = "dodge") +
  labs(title = "Graph 6: Average Family Member Numbers by Age",
       x = "Type",
       y = "Average Number",
       fill = "Age Cohort")
```
...
gss %>%
  ...
  labs(y = "Percentage") +
  scale_y_continuous(label = scales::percent) +
  labs(title = "Graph 7: Religion and Education",
       x = "Education")

ggplot(gss4, aes(degree, ideology)) +
  geom_boxplot() +
  stat_summary(fun.y=mean, geom = "point", shape = 21, size = 1, fill = "red", color = "red") +
  labs(title = "Graph 8: Ideology and Education",
       x = "Education",
       y = "Ideology Scores")
...
gss %>%
  ...
  ggplot(aes(degree, mean_happiness, fill = degree)) +
  geom_col() +
  labs(title = "Graph 9: Education and Happiness",
       x = "Education",
       y = "Happy Score")
```
