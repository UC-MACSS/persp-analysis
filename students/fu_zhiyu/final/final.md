...
On the contrary, the observational study focuses on the prices. Since the prices are the results of an equilibrium, it is impossible to be manipulated in experiments. Actually, the discrimination against the supply side is hard to study with an experiment, as it is too expensive to create host account for an apartment to rent. It can only be studied by the observational data. The results of the observational study are more economically meaningful, since it can estimate the true loss of black hosts empirically.
